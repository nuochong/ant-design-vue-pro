<template>
  <div>
    <div class="ant-pro-table-column-setting-list ant-pro-table-column-setting-list-group">
      <span class="ant-pro-table-column-setting-list-title" v-if="fixLeft.length > 0">固定在左侧</span>
      <div class="ant-tree ant-tree-icon-hide ant-tree-block-node" v-if="fixLeft.length > 0">
        <tree
          ...
          @select="onSelect">
          <!-- <span slot="title" style="color: #000" slot-scope="{ title, fixed, key }" class="ant-pro-table-column-setting-list-item">
            <div class="ant-pro-table-column-setting-list-item-title">{{ title }}</div>
            <span class="ant-pro-table-column-setting-list-item-option">
              <a-tooltip title="固定在列首">
                <a-icon type="vertical-align-top" class="fix-icon" @click="handleFixTop(key)" v-if="fixed === 'auto' || fixed === 'right'" />
              </a-tooltip>
              <a-tooltip title="不固定">
                <a-icon type="vertical-align-middle" class="fix-icon" @click="handleNoFix(key)" v-if="fixed === 'left' || fixed === 'right'" />
              </a-tooltip>
              <a-tooltip title="固定在列尾">
                <a-icon type="vertical-align-bottom" class="fix-icon" @click="handleFixBottom(key)" v-if="fixed === 'auto' || fixed === 'left'" />
              </a-tooltip>
            </span>
          </span> -->
          <span slot="switcherIcon" slot-scope="prop">
            <dataDarg></dataDarg>
            <a-checkbox v-model="prop.isShow" @change="(e)=>{onChange(e,prop)}">
            </a-checkbox>
          </span>
        </tree>
      </div>
      <span class="ant-pro-table-column-setting-list-title" v-if="fixLeft.length > 0 || fixRight.length > 0">不固定</span>
      <div class="ant-tree ant-tree-icon-hide ant-tree-block-node">
        <tree
          class="ant-tree-icon-hide ant-tree-block-node"
          draggable
          @dragenter="onDragEnter"
      ...
      <span class="ant-pro-table-column-setting-list-title" v-if="fixRight.length > 0">固定在右侧</span>
      <div class="ant-tree ant-tree-icon-hide ant-tree-block-node" v-if="fixRight.length > 0">
        <tree
          ref="trees"
          class="ant-tree-icon-hide ant-tree-block-node"
          draggable
  ...
  methods: {
    ...
    init () {
      ...
      for (const item in columnsOld) {
        ...
        const titleTmp = (child) => {
          const info = child || ''
          return <span slot="title" style="color: #000" slot-scope="{ title, fixed, key }" class="ant-pro-table-column-setting-list-item">
            <div class="ant-pro-table-column-setting-list-item-title">2222{info}</div>
            <span class="ant-pro-table-column-setting-list-item-option">
              <a-tooltip title="固定在列首">
                { (fixed === 'auto' || fixed === 'right') && <a-icon type="vertical-align-top" class="fix-icon" onClick={() => { this.handleFixTop() }} />}
              </a-tooltip>
              <a-tooltip title="不固定">
                {(fixed === 'left' || fixed === 'right') && <a-icon type="vertical-align-middle" class="fix-icon" onClick={() => { this.handleNoFix() }} />}
              </a-tooltip>
              <a-tooltip title="固定在列尾">
                {(fixed === 'auto' || fixed === 'left') && <a-icon type="vertical-align-bottom" class="fix-icon" onClick={() => { this.handleFixBottom() }} />}
              </a-tooltip>
            </span>
          </span>
        }
        const slotTemp = () => {
          return <span slot={`custom-${item}`}>
          {titleOld}
          <a-tooltip title={tip}>
        ...
        if (tip) {
          // scopedSlotsRes['title'] = 'title'
          // scopedSlotsRes['title'] = 'customTitle'
          titleRes = titleTmp(slotTemp())
        } else {
          titleRes = titleTmp()
          // scopedSlotsRes['title'] = 'title'
        }
